...
class PublicVendorParserSuite extends FunSuite {
  ...
  test("[5] vendor parser should break with iconst without valid args") {
    intercept[InvalidInstructionFormatException] {
      val insts = vp.parseString("iconst fat")
    }
  }

  test("[6] vendor parser should break with iconst without args") {
    intercept[InvalidInstructionFormatException] {
